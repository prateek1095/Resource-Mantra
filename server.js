    ...
    if (req.session && req.session.user) {
          User.findOne({email:req.session.user.email}, function(err,user){
            if(!user){
                ...
                res.locals.user = user;
                res.sendFile(path.join(__dirname+'/public/partials/projects.html'));
                console.log("testing");
             }
        });
    ...
    if (req.session && req.session.user) {
          User.findOne({email:req.session.user.email}, function(err,user){
            if(!user){
                ...
                res.locals.user = user;
                res.sendFile(path.join(__dirname+'/public/partials/workers.html'));
                console.log("testing");
             }
        });
    ...
    if (req.session && req.session.user) {
          User.findOne({email:req.session.user.email}, function(err,user){
            if(!user){
                ...
                res.locals.user = user;
                res.sendFile(path.join(__dirname+'/public/partials/accounts.html'));
                console.log("testing");
             }
        });
