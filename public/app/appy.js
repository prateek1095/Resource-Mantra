...
app.controller('NotifyController', ['notify','$scope', function (notify,$scope) {
    //var vm = this;
    console.log("Notify");
    $scope.vm=this;
    notify.get(function (data) {
        $scope.vm.notify = data;
    });
}]);
...
app.factory('users', ['$http', function ($http) {
    return {
        get: function (callback) {
            $http.get('https://api.myjson.com/bins/10wbyd').success(function (data) {
                callback( data);
            })
        }
    }
...
app.factory('notify', ['$http', function ($http) {
    return {
        get: function (callback) {
            $http.get('api.openweathermap.org/data/2.5/forecast/daily?q=London&mode=xml&units=metric&cnt=7').success(function (data) {
                callback( data);
            })
        }
    }
}]);
app.controller('WageCtrl', ['wages','$scope', function (wages,$scope) {
    //var vm = this;
    $scope.vm=this;
    wages.get(function (data) {
        $scope.vm.wages = data;
    });
}]);
app.factory('wages', ['$http', function ($http) {
    return {
        get: function (callback) {
            $http.get('https://api.myjson.com/bins/dqzyd').success(function (data) {
                callback( data);
            })
        }
    }
...
app.controller('DashboardCtrl', ['$scope','$http','$route','$location','$firebaseArray','notify', function($scope,$http ,$route,$location,$firebaseArray,notify){
     ...
     $scope.addproject = function(text) {
         $scope.texts.$add({
            ...
            priority:text.priority
                ...
                text.Title="";
                text.workers = "";//ek counter chalayenge no. of workers ka.
                text.position="";
                text.date="";
                text.priority="";
            ...
            $scope.MedicalDatefrom = "" ;
              ...
              for(var i=0 ;i<=$scope.sitedata.length;i++){
                ...
                $scope.check = new Date(c[2], parseInt(c[1])-1, c[0]);
                  ...
                  if( ($scope.sitedata[i].Position == "General Worker") && ($scope.check >= $scope.Medicalfrom && $scope.check <= $scope.Medicalto) && ($scope.check >= $scope.Expiryfrom && $scope.check <= $scope.Expiryto)){
                              ...
                              if($scope.sitedata[i]['TIME'] < $scope.sitedata[i+1]['TIME']){
                                    console.log("Check-in 1 is earlier than Check-in 2 , so worker 1 allocated");
                              }else{
                                    console.log("Check-in 2 is earlier than Check-in 1 , so worker 2 allocated");
                              }
                  }

                if( ($scope.sitedata[i].Position == "Learner Artisan: Electro Mechanic") && ($scope.check >= $scope.Medicalfrom && $scope.check <= $scope.Medicalto) && ($scope.check >= $scope.Expiryfrom && $scope.check <= $scope.Expiryto)){
                              //$scope.see = $scope.sitedata[i]['Name & Surname'];
                              console.log($scope.sitedata[i]['TIME']);
                              console.log($scope.sitedata[i].Name);
                              if($scope.sitedata[i]['TIME'] < $scope.sitedata[i+1]['TIME']){
                ...
                if( ($scope.sitedata[i].Position == "Lead Operator: Controlroom") && ($scope.check >= $scope.Medicalfrom && $scope.check <= $scope.Medicalto) && ($scope.check >= $scope.Expiryfrom && $scope.check <= $scope.Expiryto)){
                              //$scope.see = $scope.sitedata[i]['Name & Surname'];
                              console.log($scope.sitedata[i]['TIME']);
                              console.log($scope.sitedata[i].Name);
                              if($scope.sitedata[i]['TIME'] < $scope.sitedata[i+1]['TIME']){
